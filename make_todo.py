...
def make_todo(dir_path):
    files = glob.glob(os.path.join(dir_path, "*segment_*todo.txt"))
    print(files)
    ...
    for file in files:
        with open(file, 'r', encoding='utf-8') as f:
            print(file)
            data = json.load(f)
            #combined_minutes = ["□ {}/{}\n".format(item['title'], item['Priority']) for item in data['todo']]
            combined_minutes = ["□ {}\n".format(replace_words(item['title'], replacement_dict)) for item in data['todo']]
